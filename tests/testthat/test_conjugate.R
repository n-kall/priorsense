...
test_that("powerscaling diagnostic makes sense for simple model", {
  ...
  post1_draws <- as_draws_df(data.frame(
    mu = rnorm(4000, post1[["mean"]], post1[["sd"]])
  ))
  post1_loglik <- as_draws_df(data.frame(
    log_lik = vapply(post1_draws$mu, FUN = ll, FUN.VALUE = c(1), x = x)
  ))
  post1_lprior <- as_draws_df(data.frame(
    lprior = dnorm(post1_draws$mu, mean = 0, log = TRUE)
  ))
  psd1 <- create_priorsense_data(
    x = post1_draws,
    log_prior = post1_lprior,
    log_lik = post1_loglik
  )
  ps1 <- powerscale_sensitivity(psd1)

  # prior2 is mu ~ normal(5, 1)
  post2 <- post(1, sigma = sigma, mu_0 = 5, n = n, x = x)
  post2_draws <- as_draws_df(data.frame(
    mu = rnorm(4000, post2[["mean"]], post2[["sd"]])
  ))
  post2_loglik <- as_draws_df(data.frame(
    log_lik = vapply(post2_draws$mu, FUN = ll, FUN.VALUE = c(1), x = x)
  ))
  post2_lprior <- as_draws_df(data.frame(
    lprior = dnorm(post2_draws$mu, mean = 5, log = TRUE)
  ))
  psd2 <- create_priorsense_data(
    x = post2_draws,
    log_prior = post2_lprior,
    log_lik = post2_loglik
  )
  ps2 <- powerscale_sensitivity(psd2)

  # prior3 is mu ~ normal(10, 1)
  post3 <- post(1, sigma = sigma, mu_0 = 10, n = n, x = x)
  post3_draws <- as_draws_df(data.frame(
    mu = rnorm(4000, post3[["mean"]], post3[["sd"]])
  ))
  post3_loglik <- as_draws_df(data.frame(
    log_lik = vapply(post3_draws$mu, FUN = ll, FUN.VALUE = c(1), x = x)
  ))
  post3_lprior <- as_draws_df(data.frame(
    lprior = dnorm(post3_draws$mu, mean = 10, log = TRUE)
  ))
  psd3 <- create_priorsense_data(
    x = post3_draws,
    log_prior = post3_lprior,
    log_lik = post3_loglik
  )
