...
##' @export
summarise_draws.exptilted_draws <- function(.x,
                                            ...,
                                            .num_args = NULL,
                                            .args = list(),
                                            base_draws = NULL,
  ...
  et_details <- get_exptilting_details(.x)

  .args <- as.list(.args)

  if (resample && !et_details$resampled) {
    # only resample if specified and draws are not already resampled
    target_draws <- posterior::resample_draws(
      posterior::merge_chains(.x)
    )
  ...
  if (resample) {
    et_details$resampled <- TRUE
  }

  attr(out, "exptilting") <- et_details


  class(out) <- c("exptilted_draws_summary", class(out))
  return(out)
}
