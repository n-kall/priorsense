...
generated quantities { // these matrices are saved in the output but do not figure into the likelihood
  ...
  vector[len_free_c[9]] Psi_pri_c;
    ...
    log_prior += normal_lpdf(Tau_ufree | tau_primn, tau_sd);
    log_prior += normal_lpdf(Lambda_y_free_c | lambda_y_primn_c, lambda_y_sd_c);
    log_prior += normal_lpdf(B_free_c | b_primn_c, b_sd_c);
    log_prior += normal_lpdf(Nu_free_c | nu_primn_c, nu_sd_c);
    log_prior += normal_lpdf(Alpha_free_c | alpha_primn_c, alpha_sd_c);
 ...
 Theta_pri = Theta_sd_free;
  if (len_free[5] > 0 && theta_pow != 1) {
    for (i in 1:len_free[5]) {
      Theta_pri[i] = Theta_sd_free[i]^(theta_pow);
      log_prior += log(abs(theta_pow)) + (theta_pow - 1)*log(Theta_sd_free[i]);
    }
  }
  ...
  if (len_free[9] > 0 && psi_pow != 1) {
    for (i in 1:len_free[9]) {
      Psi_pri[i] = Psi_sd_free[i]^(psi_pow);
      log_prior += log(abs(psi_pow)) + (psi_pow - 1)*log(Psi_sd_free[i]);
    }
  }
  log_prior += gamma_lpdf(Theta_pri | theta_sd_shape, theta_sd_rate);
  log_prior += gamma_lpdf(Psi_pri | psi_sd_shape, psi_sd_rate);
  log_prior += beta_lpdf(.5 * (1 + Theta_r_free) | theta_r_alpha, theta_r_beta) + log(.5) * len_free[7]; // the latter term is the 
  // jacobian moving from (-1,1) to (0,1), because beta_lpdf is defined on (0,1)
  ...
  if (sum(psinblk) > 0) {
    for (k in 1:sum(psinblk)) {
      ...
      int arrayidx = psiblkse[k, 5];
      if (arrayidx == 1) {
	log_prior += lkj_corr_lpdf(Psi_r_mat_1[blkidx] | psiblkpri[k]);
      } else if (arrayidx == 2) {
	log_prior += lkj_corr_lpdf(Psi_r_mat_2[blkidx] | psiblkpri[k]);
      } else if (arrayidx == 3) {
	log_prior += lkj_corr_lpdf(Psi_r_mat_3[blkidx] | psiblkpri[k]);
      } else if (arrayidx == 4) {
	log_prior += lkj_corr_lpdf(Psi_r_mat_4[blkidx] | psiblkpri[k]);
      } else {
	log_prior += lkj_corr_lpdf(Psi_r_mat_5[blkidx] | psiblkpri[k]);
      }
    }
  }
  if (len_free[10] > 0) {
    log_prior += beta_lpdf(.5 * (1 + Psi_r_free) | psi_r_alpha, psi_r_beta) + log(.5) * len_free[10];
  }
  if (sum(thetanblk) > 0) {
    for (k in 1:sum(thetanblk)) {
      int blkidx = thetablkse[k, 6];
      int arrayidx = thetablkse[k, 5];
      if (arrayidx == 1) {
	log_prior += lkj_corr_lpdf(Theta_r_mat_1[blkidx] | thetablkpri[k]);
      } else if (arrayidx == 2) {
	log_prior += lkj_corr_lpdf(Theta_r_mat_2[blkidx] | thetablkpri[k]);
      } else if (arrayidx == 3) {
	log_prior += lkj_corr_lpdf(Theta_r_mat_3[blkidx] | thetablkpri[k]);
      } else if (arrayidx == 4) {
	log_prior += lkj_corr_lpdf(Theta_r_mat_4[blkidx] | thetablkpri[k]);
      } else {
	log_prior += lkj_corr_lpdf(Theta_r_mat_5[blkidx] | thetablkpri[k]);
      }
    }
  }
  // and the same for level 2
  ...
  if (len_free_c[5] > 0 && theta_pow_c != 1) {
    for (i in 1:len_free_c[5]) {
      Theta_pri_c[i] = Theta_sd_free_c[i]^(theta_pow_c);
      log_prior += log(abs(theta_pow_c)) + (theta_pow_c - 1)*log(Theta_sd_free_c[i]);
    }
  }
  Psi_pri_c = Psi_sd_free_c;
  if (len_free_c[9] > 0 && psi_pow_c != 1) {
    for (i in 1:len_free_c[9]) {
      Psi_pri_c[i] = Psi_sd_free_c[i]^(psi_pow_c);
      log_prior += log(abs(psi_pow_c)) + (psi_pow_c - 1)*log(Psi_sd_free_c[i]);
    }
  }
  log_prior += gamma_lpdf(Theta_pri_c | theta_sd_shape_c, theta_sd_rate_c);
  log_prior += gamma_lpdf(Psi_pri_c | psi_sd_shape_c, psi_sd_rate_c);
  if (sum(psinblk_c) > 0) {
    for (k in 1:sum(psinblk_c)) {
      int blkidx = psiblkse_c[k, 6];
      int arrayidx = psiblkse_c[k, 5];
      if (arrayidx == 1) {
	log_prior += lkj_corr_lpdf(Psi_r_mat_1_c[blkidx] | psiblkpri_c[k]);
      } else if (arrayidx == 2) {
	log_prior += lkj_corr_lpdf(Psi_r_mat_2_c[blkidx] | psiblkpri_c[k]);
      } else if (arrayidx == 3) {
	log_prior += lkj_corr_lpdf(Psi_r_mat_3_c[blkidx] | psiblkpri_c[k]);
      } else if (arrayidx == 4) {
	log_prior += lkj_corr_lpdf(Psi_r_mat_4_c[blkidx] | psiblkpri_c[k]);
      } else {
	log_prior += lkj_corr_lpdf(Psi_r_mat_5_c[blkidx] | psiblkpri_c[k]);
      }
    }
  } else if (len_free_c[10] > 0) {
    log_prior += beta_lpdf(.5 * (1 + Psi_r_free_c) | psi_r_alpha_c, psi_r_beta_c) + log(.5) * len_free_c[10];
  }
  if (sum(thetanblk_c) > 0) {
    for (k in 1:sum(thetanblk_c)) {
      int blkidx = thetablkse_c[k, 6];
      int arrayidx = thetablkse_c[k, 5];
      if (arrayidx == 1) {
	log_prior += lkj_corr_lpdf(Theta_r_mat_1_c[blkidx] | thetablkpri_c[k]);
      } else if (arrayidx == 2) {
	log_prior += lkj_corr_lpdf(Theta_r_mat_2_c[blkidx] | thetablkpri_c[k]);
      } else if (arrayidx == 3) {
	log_prior += lkj_corr_lpdf(Theta_r_mat_3_c[blkidx] | thetablkpri_c[k]);
      } else if (arrayidx == 4) {
	log_prior += lkj_corr_lpdf(Theta_r_mat_4_c[blkidx] | thetablkpri_c[k]);
      } else {
	log_prior += lkj_corr_lpdf(Theta_r_mat_5_c[blkidx] | thetablkpri_c[k]);
      }
    }
  } else if (len_free_c[7] > 0) {
    log_prior += beta_lpdf(.5 * (1 + Theta_r_free_c) | theta_r_alpha_c, theta_r_beta_c) + log(.5) * len_free_c[7];
  }
